$(function () {
	...
	function getTime() {
		...
		// console.log();
		$('#date').find('span').eq(0).text(myYear + '/' + fillZero(myMonth) + '/' + fillZero(myToday));
		// console.log($('#date').find('span').eq(1).html(week[myDay]));
		// $('#date').find('span').eq(1).html();
		$('#date').find('span').eq(1).text(week[myDay]);

		$('#time').text(fillZero(myHour) + ':' + fillZero(myMinute) + ':' + fillZero(mySecond));
	};
	...
	$.ajax({
		...
		success: function (data) {
			...
			for (var i = 0; i<= data.length -1; i++) {
				...
				point1.push([parseFloat(geo[0]),parseFloat(geo[1])])
				risk_name.push(data[i].address)
				risk_degree.push(data[i].areasDegree)
				if (data[i].areasDegree == '高风险地区'){
					conut_high++
				}else if(data[i].areasDegree == '中风险地区'){
					conut_mid++
				}else if(data[i].areasDegree == '低风险地区'){
					conut_low++
				}
			...
			area['point1'] = point1
			console.log(point1)
			area['risk_name'] = risk_name
			area['risk_degree'] = risk_degree

		}
	})

	console.log(conut_high)
	console.log(conut_low)
	console.log(conut_mid)
	// $('#indicator3').val(conut_high)
	$('#indicator3').attr('total',conut_high);
	$('#indicator2').attr('total',conut_low);
	$('#indicator1').attr('total',conut_mid);

	var maker;


	for (var i = 0; i < area['point1'].length-1; i += 1) {
		if ( !Object.is(area['point1'][i][1], NaN) && area['risk_degree'][i] != '低风险地区'){
			var marker = new AMap.Marker({
				position: area['point1'][i],
				map: myMap,
				icon: '/risk/images/risk_high.png',
			});
			marker.content = '<p>地址：'+ area['risk_name'][i] +'</p>' +
				'<p>坐标：'+ area['point1'][i] +'</p>' +
				'<p>当前状态：'+ area['risk_degree'][i] +'</p>';
			marker.on('click', markerClick);
		}else {
			// console.log(area['point1'][i])
		}
	...
	// FontScroll
	//运单状态文字滚动
	// $('#FontScroll').FontScroll({ time: 500, speed: 1000, num: 1, skip: 1, init: false });
	...
	setTimeout(function () {
		...
		//基本信息
		totalNum($('#indicator1'), 100);
		totalNum($('#indicator2'), 100);
		totalNum($('#indicator3'), 100);
	...
	//车辆信息弹出框的显示隐藏效果
	// $('.shebeiinfo').on('click', function () {
	// 	demo.pause();
	// 	$('.filterbg').show();
	// 	$('.carInfo').show();
	// 	$('.carInfo').width('3px');
	// 	$('.carInfo').animate({ height: '76%' }, 400, function () {
	// 		$('.carInfo').animate({ width: '52%' }, 400);
	// 	});
	// 	setTimeout(function () {
	// 		$('.infoBox').show();
